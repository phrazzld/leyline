...
- [ ] Create `lib/leyline/cache/file_cache.rb` with class skeleton including initialize method that accepts `cache_dir` parameter defaulting to `~/.leyline/cache`
- [ ] Add `require 'digest'` and `require 'fileutils'` to file_cache.rb for SHA256 hashing and directory operations
- [ ] Implement `ensure_directories` private method that creates `@content_dir = #{@cache_dir}/content` with FileUtils.mkdir_p
