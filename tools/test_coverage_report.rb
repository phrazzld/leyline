...
def analyze_file_coverage(file_path)
  ...
  test_file = file_path.gsub(/\.rb$/, '_test.rb').gsub(/tools\//, 'tools/test_')
  alt_test_file = file_path.gsub(/tools\//, 'tools/test_')
...
def identify_missing_coverage(file_path, coverage_percentage)
  ...
  if coverage_percentage < 70
    missing_areas << "Core functionality"
  end
...
def format_text_report(report)
  ...
  if summary[:meets_threshold]
    output << "**Status:** ✅ PASSING (meets threshold)"
  else
    output << "**Status:** ❌ FAILING (below threshold)"
  end
  ...
  if low_coverage_files.empty?
    ...
    low_coverage_files.each do |file_coverage|
      ...
      unless file_coverage[:has_tests]
        output << "  - Create test file: `test_#{File.basename(file_coverage[:file])}`"
      end
...
def main
  ...
  if report[:summary][:meets_threshold]
    ...
    puts "\n❌ Coverage below threshold (#{report[:summary][:overall_coverage]}% < #{$options[:threshold]}%)" unless $options[:output_file]
    exit 1
  end
end
