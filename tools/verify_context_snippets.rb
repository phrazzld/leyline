...
test 'context shows correct lines around error' do
  ...
  unless output_with_context.include?('last_modified: Invalid-Date-Format')
    raise 'Should show line 3 content (last_modified)'
  end
  unless output_with_context.include?('derived_from: nonexistent-tenet')
    raise 'Should show line 4 content (derived_from)'
  end
end
...
test 'no context for files without content' do
  ...
  raise 'Could not isolate other-file.md section for testing' unless start_idx && end_idx && start_idx < end_idx
